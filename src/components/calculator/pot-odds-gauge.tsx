...
export function PotOddsGauge({ potOdds, equity }: PotOddsGaugeProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="relative h-10 sm:h-10 rounded-full overflow-hidden bg-white/[0.06] border border-white/10">
        <div
          className="absolute inset-y-0 left-0 bg-red-500/25 transition-all duration-300"
          style={{ width: `${clampedPotOdds}%` }}
        />
        <div
          className="absolute inset-y-0 bg-green-500/25 transition-all duration-300"
          style={{ left: `${clampedPotOdds}%`, right: 0 }}
        />

        <div
          className="absolute top-0 bottom-0 w-0.5 bg-white/60 z-10 transition-all duration-300"
        ...
        >
          <div className="absolute -top-6 left-1/2 -translate-x-1/2 whitespace-nowrap text-[10px] text-white/50 font-medium">
            {t("potOdds.gauge.need")} {clampedPotOdds.toFixed(1)}%
          </div>
        </div>
        ...
        >
          <div
            className={cn(
              "w-7 h-7 rounded-full border-2 flex items-center justify-center text-[10px] font-bold shadow-lg transition-all duration-200",
              isProfitable
      ...
      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-white/40">{t("potOdds.gauge.foldZone")}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-green-500/30 border border-green-500/50" />
          <span className="text-white/40">{t("potOdds.gauge.callZone")}</span>
        </div>
      </div>
    </div>
  );
